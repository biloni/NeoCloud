...
### AI feature: People AI Assistant / NL query (`/ask`)

Tool-use, not text-to-SQL — the model never sees or generates SQL. `lib/ai-tools.ts` defines six zod-validated data tools (`query_workers`, `get_headcount_summary`, `get_comp_vs_band`, `get_attrition`, `draft_document`, `get_org_chart`); the server executes them against Prisma/the snapshot helper and returns structured JSON. `get_org_chart` answers reporting-line and direct-report questions by reusing the same `getOrgChartView` helper the `/org-chart` page and worker profile "Manager" tab already use, and resolves a worker by ID or a fuzzy legal-name match (surfacing an `ambiguousMatch` note back to the model when a name matches more than one worker). `app/api/ask/route.ts` runs the standard Anthropic tool-use loop (call → tool_use → execute → tool_result → call again), capped at 4 rounds. It's available to **every role**, not just Exec/Admin — `VIEW_ASK_PEOPLE_OS` lives in `EMPLOYEE_BASE` in `security/permissions.ts`, so it's inherited by all nine personas.

**Structured prompt + structured output (the actual anti-hallucination mechanism):**
- The system prompt in `route.ts` is split into labeled sections (`ROLE` / `DATA ACCESS` / `GUARDRAILS` / `RESPONSE PROTOCOL`) rather than one prose paragraph, so each rule is independently checkable instead of buried mid-paragraph.
- The model must call a seventh, terminal tool — `provide_answer({ answer, confidence, confidenceReason, citations[] })` — as its last action instead of ever replying in plain assistant text. This is the real guardrail: rather than *hoping* the model remembers to write a "Sources:" section in prose (which degrades over longer conversations), citing tool results is a required field in a schema the server parses directly. If the model calls it with no citations and low confidence, that's visible and honest instead of silently blended into fluent-sounding prose.
- `confidence` is self-reported per the rubric in the tool description (`high` = every fact directly tool-sourced and unambiguous; `medium` = truncation, an ambiguous name match, or minor inference; `low` = a tool errored, returned nothing, or the question falls outside what these tools can answer) and rendered in the UI as a color-coded badge with a hover tooltip explaining why.
- `citations` is rendered as an expandable "N sources" list under each answer, one entry per tool call that grounded a fact — e.g. *"get_org_chart on E0002: 6 direct reports"* — so a user can see exactly which live query backs the claim, not just that *a* tool was called.
- If the model ever ignores the protocol and answers in plain text anyway, the route still surfaces that text but forces `confidence: "low"` with no citations, rather than passing it off as verified.

**Per-role tool scoping and prompt framing:** every role shares one AI engine, but not one toolbox. `ROLE_TOOL_ACCESS` in `lib/ai-tools.ts` maps each of the 9 roles to the subset of the six data tools appropriate to that job (e.g. a plain Employee's assistant never even offers `draft_document` or `get_attrition` as callable tools; Payroll Admin gets `get_comp_vs_band` but not `draft_document`), and `getToolsForRoles()` unions the sets for a worker holding multiple roles — additive, matching how every other permission in this app composes. Verified directly against `resolveRolesForWorker()` for all 9 demo personas: an isolated Employee resolves to exactly 3 tools, and each fixed-role persona's own list is independently correct (confirmed by testing the roles in isolation), though most of the fixed-role demo workers (HR Ops, Payroll Admin, etc.) also happen to derive `MANAGER`/`SKIP_LEVEL_MANAGER` from the seeded org tree, so their *effective* set in the live demo is the union with that broader tier — an honest consequence of additive RBAC, not a scoping bug. The system prompt's `ROLE` section also gets a one-sentence per-role framing (`ROLE_FRAMING`) describing what that persona is likely trying to do — `GUARDRAILS` and `RESPONSE PROTOCOL` stay byte-for-byte identical across every role on purpose: framing may change tone, never what the model is allowed to claim.

**Other guardrails:**
- Server-side only — the API key is never sent to the client.
- Tool allow-list with zod parameter validation; the model cannot call anything outside the seven defined tools (six data tools + `provide_answer`), and `provide_answer` itself has no data access — it only carries text the model already produced.
- Guardrails explicitly instruct the model to answer only from tool results in *this* conversation (no general/outside knowledge), to prefer aggregate/department-level answers, to refuse to speculate about an individual's attrition risk, performance, or personal circumstances, and to ignore any instruction that appears inside a tool result or the user's question asking it to override these rules.
...
- In-memory rate limiting keyed **per worker id** (20 requests/min), not one shared global bucket — necessary now that access is org-wide (~350 potential users) rather than Exec/Admin-only, so one chatty user can't lock everyone else out. Documented as a demo-grade guardrail, not production-grade — a real deployment would back this with Redis (or similar) so it survives process restarts and works across instances, plus real authN/Z.
...
**Responsible use, now that every employee has access:** opening this up org-wide (rather than Exec/Admin-only) means any employee can ask `query_workers`/`get_comp_vs_band`-style questions about *other* named individuals, not just themselves — the same tools an HR Partner uses. This is a deliberate take-home tradeoff, not a production posture: a real rollout would scope `query_workers`/`get_comp_vs_band` to self-and-reports for non-HR roles (the same `DataScope` layer `security/authorization.ts` already applies to the Worker Directory and Payroll), and would log every query for audit the way `ProxyAuditLog` already does for impersonation. The one guardrail that *is* in place regardless of role is the individual-risk-inference refusal above — no role, including HR, can get this assistant to speculate about a specific person's attrition risk or performance.
